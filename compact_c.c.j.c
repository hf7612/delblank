...
static int compact8(char *s, int *lenI, int *chg) { //int len = *lenI; int beg=0; int end=0; //*chg = 0; //deB//printf("%s\n", s);
    ...
    while(SYM[i]){
        ...
        while(t = strstr(sR, SYM[i]) ){ //printf(" %d ", t-s);
            ...
            *chg = 1; }
        i++; }} // extern  
static compac_cont_space(char *s,int *lenI, int *chg){
    // int i = 0; int j = 0;
    // if(*lenI<2)
    //     return -1;
    // for(i=0; i<lenI; i++){
    //     if(isspace(s[i])){ j = i+1;
    //         for(j; j<lenI; j++){
    //             if(isspace(s[j])){
    //                 s[j] = 0;
    //                 *chg = 1;
...
static int compact9(char *s, int *lenI, int *chg) { int i = 0; 
    for(i=0; i<lenI; i++){ int iK = 0;
        if(isspace(s[i])){ continue; }//skip begin blank
        int j=i+1;
        for(j; j<lenI; j++){
            if(s[j] == '\n'){  int l = j-i; 
                if(l > 1)
                    compac_cont_space(s+i, &l, chg);
                i=j+1;
                break;} } } }
// static int compact4(char *s, int *lenI, int *chg) { int len = *lenI; int beg=0; int end=0; //*chg = 0; //deB//printf("%s\n", s);
//     char *t = 0;//g ets(s);
        ...
        if(fSz > 0){
            ...
            if(pF){ //deB 
                ...
                if(gS){ //deB
                    ...
                    compact7(gS, &r, &chg);
                    compact8(gS, &r, &chg);
                    compact9(gS, &r, &chg); fprintf(stdout, "%s", gS);
                    // if(chg){// do_md5(gS, r);
                    //     fseek(pF, 0L, SEEK_SET); printf(" %s\n", f);//deBV("%gS", f);
                    //     fwrite(gS+1, 1, r, pF); } // int ftruncate(int fd, off_t length); // printf(" %s ", gS); // fwrite(gS, 1, statbuf.st_size, stdout);
                    free(gS);

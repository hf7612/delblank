...
//             *chg = 1;// s[i] = 0;
//         } } }
// static int trim_EXPORT_SYMBOL_0(char *s, int *lenI, int *chg) { int len = *lenI; int beg=0; int end=0; //*chg = 0; //deB//printf("%s\n", s);
//     char *t = 0;//g ets(s);
//     char *t2 = 0; int ind0=0,ind1=0,ind2=0,ind3=0;
//     #define EXP_SYM "\nEXPORT_SYMBOL"
//     char *sR = s; int offL = strelen(EXP_SYM);
//     while(t = strstr(sR, EXP_SYM)){ //printf(" %d ", t-s);
//         sR = t+offL;
...
static int trim_EXPORT_SYMBOL_0(char *s, int *lenI, int *chg) { int len = *lenI; int beg=0; int end=0; //*chg = 0; //deB//printf("%s\n", s);
    char *t = 0;//g ets(s);
    char *t2 = 0; int ind0=0,ind1=0,ind2=0,ind3=0;
    #define EXP_SYM "EXPORT_SYMBOL"
    char *sR = s; int offL = strlen(EXP_SYM);
    while(t = strstr(sR, EXP_SYM)){ //printf(" %d ", t-s); 
        ...
        sR = t+offL; } }
char *gS = 0;
static int trim_EXPORT_SYMBOL(char *f){ struct stat statbuf; //deB
...
int main(int argc, char **argv) { int c; int digit_optind = 0; //deB
    while (1) { int this_option_optind = optind ? optind : 1; int option_index = 0;
        ...
        case 'f':  //printf("option f with value '%s'\n", optarg);
            trim_EXPORT_SYMBOL(optarg); //trimKmsg(optarg);

...
static int compact0(char *s, int *lenI, int *chg) { //int len = *lenI; int beg=0; int end=0; //*chg = 0; //deB//printf("%s\n", s);
    char *t = 0;// char *t2 = 0; int ind0=0,ind1=0,ind2=0,ind3=0;   
    char *sR = s;  
    while((t = strchr(sR, '\n'))){ //printf(" %d ", t-s);
        sR = t+1;
...
static int compact2(char *s, int *lenI, int *chg) { int len = *lenI; int beg=0; int end=0; //*chg = 0; //deB//printf("%s\n", s);
    ...
    T:
    while(t = strstr(sR, SYM2) ){ sR = t+offL;//printf(" %d ", t-s);
        for(int i=t-s-1; i>=0; i--){
            if(s[i] == '\n' )
                break;
            if(s[i] =='/' || s[i] == '#'){ goto T;
...
*/
static int compact2b(char *s, int *lenI, int *chg) { int len = *lenI; int i=0; int specTagPos=-1;
    T:
    for(i; i<len; i++){
        if(s[i] =='/' || s[i] == '#'){ specTagPos=i+1; i++;
            for(i; i<len; i++){
                if(s[i]=='\n'){ specTagPos=i+1;
                    break; } }
            continue;}
        if(s[i]=='}'){
            for(int j=i-1; j>=specTagPos; j--){
                if(s[j] == '\n' ){ s[j]=' '; *chg = 1; 
                    break;}
                if(!isspace(s[j])){ i++; goto T; } 
            }        } }
    return 0; }
    // while(t = strstr(sR, SYM2) ){ sR = t+offL;//printf(" %d ", t-s);
    //     for(int i=t-s-1; i>=0; i--){
